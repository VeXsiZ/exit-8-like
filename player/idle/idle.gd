...
func rules_checker():
	if player_ref.is_on_floor():
		if ["forward","backward","left","right"].any(Input.is_action_pressed):
			state_transition.emit("walking")
		elif ["forward","backward","left","right"].any(Input.is_action_pressed) and Input.is_action_pressed("running"):
			state_transition.emit("running")
		elif Input.is_action_pressed("jump"):
			state_transition.emit("mid-air")
...
func moving(delta):
	...
	if player_ref.is_on_floor():
		...
		player_ref.velocity.x = lerp(player_ref.velocity.x, direction.x * SPEED, delta * 3.0)
		player_ref.velocity.z = lerp(player_ref.velocity.z, direction.z * SPEED, delta * 3.0)
